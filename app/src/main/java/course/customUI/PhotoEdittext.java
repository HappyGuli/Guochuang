...
public class PhotoEdittext extends EditText {
    ...
    public PhotoEdittext(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public PhotoEdittext(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (mContentList.size() > 0) {
            for (String str : mContentList) {
                if (str.indexOf(mBitmapTag) != -1) {//判断是否是图片地址
                    ...
                    Bitmap bitmap = getSmallBitmap(path, 360, 600);
                    //插入图片
    ...
    private SpannableString insertBitmap(String path, Bitmap bitmap) {
        ...
        path = mBitmapTag + mBitmapTag_pre + path + mBitmapTag;
    ...
    public List<String> getmContentList() {
        ...
        //将所有‘\n’转换为 <br /> 以便于在textView中以html的方式展示
        String content = getText().toString().replaceAll(" ", "&nbsp;");
        content = content.replaceAll(mNewLineTag, "<br />");
